<!DOCTYPE html>

<head>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Raleway:wght@400;500;700&display=swap" rel="stylesheet">

    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/x-icon" href="img/urslogo.png">


    <link rel="stylesheet" href="styles/admin-forgot-pass.css">
    <title>URS Archive | Forgot Password</title>

</head>

<body>

    <div class="section-grid">
        <div class="left-section">
            <div>
                <img src="img/urslogo.png" alt="" class="logo">
            </div>
            <div class="welcome-sidebar">
                <div class="welcome-label">URS Archive</div>
                <div class="welcome-statement">Access most of the theses accomplished by University of Rizal System - Antipolo Campus students and alumnis.</div>
            </div>
        </div>
        <div class="right-section">
            <div class="header">
                <div class="welcome-label-header-container">
                    <div class="welcome-label-header">URS Archive</div>
                    <div class="welcome-statement-header">Access most of the theses accomplished by University of Rizal System - Antipolo Campus students and alumnis.</div>
                </div>
            </div>
            <form action="">
                <div class="fp-form-container">
                    <div class="fp-label-container">
                        <p class="fp-label">FORGOT PASSWORD</p>
                    </div>
                    <div class="fp-form">
                        <div class="fp-input-container" style="margin-bottom: 10px;">Please enter your email address to recover your account
                        </div>

                        <div class="fp-input-container">
                            <input type="text" class="fp-input-email" placeholder="Email address" required>
                        </div>

                        <div class="buttons-container">
                            <div class="fp-input-container">
                                <div class="fp-button-container">
                                    <button class="fp-button fp-button-reset" type="submit">RESET PASSWORD</button>
                                </div>
                            </div>

                            <div class="fp-input-container">
                                <div class="fp-button-container">
                                    <a href="admin-signin.php" class="fp-button fp-button-back">BACK</a>
                                </div>
                            </div>
                        </div>

                    </div>
                </div>
            </form>
        </div>
    </div>

</body>

</html>